...
| `FPRD-CAT001-AC-002` | RQ-001, RQ-003 | `CAT-APP-002` | application | `CAT-DB-001` unique slug | `PARTIAL_RED_CONFIRMED` |
| `FPRD-CAT001-AC-003` | RQ-005 | `CAT-DOM-001` | domain | explicit order fixture | `RED_CONFIRMED` |
| `FPRD-CAT001-AC-004` | RQ-006, RQ-007 | `CAT-DOM-002` | domain | `CAT-DB-002` concurrent unique constraint | `PARTIAL_RED_CONFIRMED` |
| `FPRD-CAT001-AC-005` | RQ-007 | `CAT-DOM-003` | domain | `CAT-DB-003` global unique constraint | `PARTIAL_RED_CONFIRMED` |
| `FPRD-CAT001-AC-006` | RQ-008 | `CAT-DOM-004` | domain | exact decimal cases | `RED_CONFIRMED` |
| `FPRD-CAT001-AC-007` | RQ-008 | `CAT-DOM-005` | domain | `CAT-DB-004` partial unique barcode | `PARTIAL_RED_CONFIRMED` |
...
| `FPRD-CAT001-AC-011` | RQ-010 | `CAT-APP-003` | application | `CAT-DB-005` atomic transaction | `PARTIAL_RED_CONFIRMED` |
...
| `FPRD-CAT001-AC-014` | RQ-014 | `CAT-APP-004` | application | `CAT-DB-006` cursor query | `PARTIAL_RED_CONFIRMED` |
...
| `FPRD-CAT001-AC-017` | RQ-017 | `CAT-SEC-002` | Supabase integration | schemas + grants + RLS baseline | `BLOCKED_ENV` |
...
| Test | Blocking evidence | Minimum unblock |
|---|---|---|
| `CAT-DB-001..006` | PostgreSQL 13/17 exists locally but requires an unavailable credential; target Supabase project is not visible | provide an isolated authorized database target, preferably Supabase branch/local environment |
| `CAT-SEC-002` | connector account does not list `olkadbgumpiybehslobk` | add the connected Supabase account as project/organization member |
| Storage API integration | target bucket/configuration cannot be inspected | authorize target project and run read-only baseline |

No blocked test may be marked passed from a mock. Fakes validate application orchestration only.
